...
void Display4() {
	...
	double b = 0.2;
	double pi = 4 * atan(1.0);
	double xmax = (2 * (a * cos(0) + b) * cos(0)) + 0.2;
	double ymax = (2 * (a * cos(1) + b) * sin(1)) + 0.2;
	double ratia = 0.05;
	...
	for (double t = -pi+ratia; t < pi; t += ratia) {
		double x1 = (2 * (a * cos(t) + b) * cos(t)) / xmax;
		double y1 = (2 * (a * cos(t) + b) * sin(t)) / ymax;
		glVertex2f(x1, y1);
...
void Display7() {
	...
	{
		double x1 = ((R + r) * cos(r * t / R) - r * cos(t + r * t / R));
		double y1 = ((R + r) * sin(r * t / R) - r * sin(t + r * t / R));
